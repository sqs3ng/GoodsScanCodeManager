...
//在do_Alayout_root上动态添加子视图，该视图初始化的脚本会将自己先隐藏
do_Alayout_root.add("selectType","source://view/feedback/selectType.ui");
//实例化
...
do_TextField_title.on("textChanged", function() {
	if (do_TextField_title.text.length>0 && !do_Label_title.visible) {
...
do_TextBox_content.on("textChanged",function(){
	if (do_TextBox_content.text.length>0 && !do_Label_content.visible) {
...
do_Page.on("loaded",function(){
	feedbackType="问题反馈";
	do_ALayout_type.text=feedbackType;
